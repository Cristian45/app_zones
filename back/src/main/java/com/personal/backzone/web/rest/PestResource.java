...
public class PestResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new pestDTO, or with status {@code 400 (Bad Request)} if the pest has already an ID.
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated pestDTO,
     * or with status {@code 400 (Bad Request)} if the pestDTO is not valid,
     * or with status {@code 500 (Internal Server Error)} if the pestDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PutMapping("/pests/{id}")
    public ResponseEntity<PestDTO> updatePest(@PathVariable(value = "id", required = false) final Long id, @RequestBody PestDTO pestDTO)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, pestDTO.getId().toString()))
            .body(result);
    }
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the updated pestDTO,
     * or with status {@code 400 (Bad Request)} if the pestDTO is not valid,
     * or with status {@code 404 (Not Found)} if the pestDTO is not found,
     * or with status {@code 500 (Internal Server Error)} if the pestDTO couldn't be updated.
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/pests/{id}", consumes = { "application/json", "application/merge-patch+json" })
    public ResponseEntity<PestDTO> partialUpdatePest(
        @PathVariable(value = "id", required = false) final Long id,
        @RequestBody PestDTO pestDTO
    ) throws URISyntaxException {
     ...
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the pestDTO, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/pests/{id}")
